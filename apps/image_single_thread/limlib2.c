...
static int image_load(lua_State *L)
{
  ...
  push_Image(L, imlib_load_image_with_error_return(path, &err));
...
static int image_putdata(lua_State *L) {
  Image *imp = (Image*)luaL_checkudata(L, 1, "imlib2.image");
  ...
  else
  {
   Image im = *imp;
   imlib_context_set_image(im);
   unsigned int * ptr=lua_touserdata(L,2);
...
static int image_from_str(lua_State *L) {
  Image *imp = (Image*)luaL_checkudata(L, 1, "imlib2.image");
  size_t len=0;
  ...
  else
  {
   ...
   unsigned int * ptr=imlib_image_get_data();
   memcpy(ptr,str,(len<img_size?len:img_size));
   imlib_image_put_back_data(ptr);
   lua_pushnumber(L,len<img_size?len:img_size);
  }
  return 1;
}


static int image_to_str(lua_State *L) {
  Image *imp = (Image*)luaL_checkudata(L, 1, "imlib2.image");
  ...
  else
  {
   ...
   imlib_context_set_image(im);
   size_t img_size=imlib_image_get_width() * imlib_image_get_height() * 4;
   char * str=malloc(img_size);
   memcpy(str,imlib_image_get_data_for_reading_only(),img_size);
   lua_pushlstring(L,str,img_size);
  }
  return 1;
}

static int image_from_rgb(lua_State *L) {
  Image *imp = (Image*)luaL_checkudata(L, 1, "imlib2.image");
  unsigned char *buf=lua_touserdata(L,2);
  if (imp != NULL)  {
   Image im = *imp;
   imlib_context_set_image(im);
   int h=imlib_image_get_height(),w=imlib_image_get_width();
   unsigned char * out=(unsigned char *)imlib_image_get_data();
	int pix;
	unsigned char * r, * b, * g;
	unsigned char * r2, * b2, * g2, *a2;
  	r=buf+0;
  	g=buf+1;
  	b=buf+2;
   ...
   imlib_image_put_back_data((void *)out);
   free(buf);
  }
  return 0;
}

static int image_free_rgb(lua_State *L) {
  unsigned char *buf=lua_touserdata(L,1);
  free((void *)buf);
  return 0;
}

static int image_to_rgb(lua_State *L) {
  Image *imp = (Image*)luaL_checkudata(L, 1, "imlib2.image");
  ...
  else
  {
   ...
   unsigned char * buf=(unsigned char *)imlib_image_get_data_for_reading_only();
   unsigned char * out=malloc(buf_size);
	int pix;
	unsigned char * r, * b, * g;
	unsigned char * r2, * b2, * g2;
  	r=buf+0;
  	g=buf+1;
  	b=buf+2;
...
static int imagem_crop(lua_State *L)
{
  ...
  old_im = *imp;
  imlib_context_set_image(old_im);
  push_Image(L,imlib_create_cropped_image(x, y, w, h));
  return 1;
}
...
static const struct luaL_Reg image_m [] = {
  ...
  {"to_str", image_to_str},
  {"from_str", image_from_str},
  {"to_rgb", image_to_rgb},
  {"from_rgb", image_from_rgb},
  {"get_data", image_getdata},
  {"put_data", image_putdata},
